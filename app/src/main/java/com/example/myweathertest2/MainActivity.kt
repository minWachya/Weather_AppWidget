...
class MainActivity : AppCompatActivity() {
        ...
        call.enqueue(object : retrofit2.Callback<WEATHER> {
            ...
            override fun onResponse(call: Call<WEATHER>, response: Response<WEATHER>) {
                if (response.isSuccessful) {
                    ...
                    val weatherArr = Array(6, {ModelWeather()})
                    var index = 0

                    for (i in 0..9) {
                        for (j in 0..5) {
                            index = (it[j].fcstTime.toInt() - base_time.toInt()) / 100 - 1
                            if (index < 0) index += 24
                            Log.d("mmm", index.toString() + ", " + j)

                            when(it[i].category) {
                                "PTY" -> weatherArr[index].rainType = it[j].fcstValue     // 강수 형태
                                "REH" -> weatherArr[index].humidity = it[j].fcstValue     // 습도
                                "SKY" -> weatherArr[index].sky = it[j].fcstValue          // 하늘 상태
                                "T1H" -> weatherArr[index].temp = it[j].fcstValue         // 기온
                                else -> continue
                            }
                        }
                    }
                    // 날씨 정보 텍스트뷰에 보이게 하기
                    //setTextView(weatherArr)
    ...
    // 텍스트 뷰에 날씨 정보 보여주기
    fun setTextView(weatherArr : Array<ModelWeather>) {
//        // 강수 형태
//        var result = ""
//        when(rainType) {
//            "0" -> result = "없음"
